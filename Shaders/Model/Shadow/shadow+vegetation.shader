...
void main()
{
	...
	//SWAY
	float seed = mod(currenttime / 100.0 * 0.25,360.0);
	seed += entitymatrix_[3].x*33.0 + entitymatrix_[3].y*67.8 + entitymatrix_[3].z*123.5;
	seed += vertex_position.x + vertex_position.y + vertex_position.z;
	vec4 movement = vec4( vec3( 0.01 * (sin(seed)+0.025*cos(seed*5.2+3.2)) ),0.0);

	ex_texcoords0 = vertex_texcoords0;
	
	vec4 modelvertexposition = entitymatrix_ * vec4(vertex_position+movement.xyz,1.0);
	gl_Position = projectioncameramatrix * modelvertexposition;
}
@OpenGL4.Fragment
#version 400
...
void main(void) {
	if (texture(texture4,ex_texcoords0).a < 0.5) discard;
}
