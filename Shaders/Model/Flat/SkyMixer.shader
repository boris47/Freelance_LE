...
uniform samplerCube texture1;//cube map
uniform vec3 cameraposition;
uniform int decalmode;
uniform float materialroughness;

// User controlled
uniform float fPhaseFade = 0.0;
uniform vec3 vSkyColor = vec3(1.0);
...
void main(void)
{
	...
	vec4 outcolor = ( ( texture(texture0,cubecoord) * ( 1 - fPhaseFade ) ) + ( texture(texture1,cubecoord) * fPhaseFade ) );
	outcolor.rgb *= vSkyColor;
