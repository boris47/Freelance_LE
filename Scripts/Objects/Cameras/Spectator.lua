...
--Player Movement
	...
	if window:KeyDown(Key.Space)		then self.input[2]=self.input[2]+1 end
	if window:KeyDown(Key.ControlKey)	then self.input[2]=self.input[2]-1 end
	
	local cameraMovement = Vec3(0)
	cameraMovement.x = self.input[0] * self.moveSpeed
	cameraMovement.y = self.input[2] * self.moveSpeed
	...
	self.entity:Move( cameraMovement.x, cameraMovement.y, cameraMovement.z )
end